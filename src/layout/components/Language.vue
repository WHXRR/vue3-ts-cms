...
const changeSystemLanguage = (lang: string) => {
  locale.value = lang
  systemStore.systemLanguage = lang
  useCache.setItem("cmsLang", lang)
}
</script>
<template>
  <a-dropdown placement="bottom">
    <a class="ant-dropdown-link" @click.prevent>
      <icon :style="{ color: systemStore.systemThemeColor.color }">
        <template #component>
          <svg
            t="1694594701178"
            class="icon"
    ...
    <template #overlay>
      <a-menu>
        <a-menu-item>
          <a href="javascript:;" @click="changeSystemLanguage('zh')">简体中文</a>
        </a-menu-item>
        <a-menu-item>
          <a href="javascript:;" @click="changeSystemLanguage('en')">English</a>
        </a-menu-item>
      </a-menu>
    </template>
  </a-dropdown>
